...
public class TarCServiceImpl extends AbstractFileHandle {
    private static Logger logger = LoggerFactory.getLogger(TarCServiceImpl.class.getName());
    private static final TarCServiceImpl INSTANCE = new TarCServiceImpl();
    private Map<String,String> sourceToTarget = new HashMap<>();
    private static Queue<String> commands = new ConcurrentLinkedQueue<>();
    public TarCServiceImpl() {
        //這裡透過建構子抓取properties資料
        this.commands = getCommand();
    }

    public TarCServiceImpl getInstance() {
        return INSTANCE;
    }
    @Override
    ...
    public Queue<String> getCommand() {
        for (Map.Entry<String, String> entry : sourceToTarget.entrySet()) {
            ...
            String command = Action.TAR_CXVF.getCommand() ;
            command = command
    ...
    private Tar getTar() {
        return (Tar) ConfigService.getInstance().getConfigYmlClass("move");
    }
}
